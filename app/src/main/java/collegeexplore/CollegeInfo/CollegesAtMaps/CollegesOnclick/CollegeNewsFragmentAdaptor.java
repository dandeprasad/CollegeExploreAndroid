...
public class CollegeNewsFragmentAdaptor extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
...
private ArrayList<Bitmap> imagedata;
    ...
    public CollegeNewsFragmentAdaptor(Context contextForFragment, ArrayList<HashMap> maincontents, ArrayList<Bitmap> imageId, OnItemClickListener listener) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if(viewType==0){
            return new NewsViewHolder(inflater.inflate(R.layout.clgclk_news_main_layout,parent,false));
        }else{
...
/*            if (position==0){
                ...
                holder.imageView.setPadding(0,0,0,0);
                        ...
                        if(!values.isEmpty()){
                            ...
                            if (values.get("FULL_IMAGE").equalsIgnoreCase("Y")) {
                                ((NewsViewHolder) holder).mViewGroup.setVisibility(View.GONE);
                                ((NewsViewHolder) holder).newsimgtagy.setVisibility(View.VISIBLE);
                              ...
                              //  Picasso.with(contextForFragmentadp).load("https://s3.us-east-2.amazonaws.com/elasticbeanstalk-us-east-2-083183914236/CLGS_PROJECT/AllNewsImages/news4.jpg").into( ((NewsViewHolder) holder).newsimgtagy);
                               // Picasso.with(contextForFragmentadp).load("http://192.168.43.60:9544/CollegeGuideWorkSpace/WebUserRegistration/news0.jpg").into( ((NewsViewHolder) holder).newsimgtagy);
                                Picasso.with(contextForFragmentadp).load(values.get("NEWSID")).into( ((NewsViewHolder) holder).newsimgtagy);
                            }
                            else {
                                ((NewsViewHolder) holder).mViewGroup.setVisibility(View.VISIBLE);
                                ((NewsViewHolder) holder).newsimgtagy.setVisibility(View.GONE);
                              //  ((NewsViewHolder) holder).imageView.setImageBitmap(imagedata.get(position));
                               // Picasso.with(contextForFragmentadp).load("https://s3.us-east-2.amazonaws.com/elasticbeanstalk-us-east-2-083183914236/CLGS_PROJECT/AllNewsImages/news4.jpg").into( ((NewsViewHolder) holder).imageView);
                                ...
                                Picasso.with(contextForFragmentadp).load(values.get("NEWSID")).into( ((NewsViewHolder) holder).imageView);
                            }
