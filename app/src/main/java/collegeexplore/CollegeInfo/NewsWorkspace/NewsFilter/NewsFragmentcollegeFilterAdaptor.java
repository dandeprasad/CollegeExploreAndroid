...
public class NewsFragmentcollegeFilterAdaptor extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
...
private ArrayList<Bitmap> imagedata;
    ...
    public NewsFragmentcollegeFilterAdaptor(Context contextForFragment, ArrayList<HashMap> maincontents, ArrayList<Bitmap> imageId, OnItemClickListener listener) {
...
/*            if (position==0){
                ...
                ((NewsViewHolder) holder).label.setText(values.get("NEWS_DETAILS"));
                ((NewsViewHolder) holder).newsmaintext.setText(values.get("NEWS_HEADER"));
                            ...
                            try {
                              ...
                              //  ((NewsViewHolder) holder).imageView.setImageBitmap(imagedata.get(position));
                               // Picasso.with(contextForFragmentadp).load("https://s3.us-east-2.amazonaws.com/elasticbeanstalk-us-east-2-083183914236/CLGS_PROJECT/AllNewsImages/news4.jpg").into( ((NewsViewHolder) holder).imageView);
                                ...
                                Picasso.with(contextForFragmentadp).load(values.get("NEWS_IMAGE")).into( ((NewsViewHolder) holder).imageView);
