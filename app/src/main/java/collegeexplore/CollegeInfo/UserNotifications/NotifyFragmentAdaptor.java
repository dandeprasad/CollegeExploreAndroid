...
public class NotifyFragmentAdaptor extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
...
private ArrayList<Bitmap> imagedata;
    ...
    public NotifyFragmentAdaptor(Context contextForFragment, ArrayList<HashMap> maincontents, ArrayList<Bitmap> imageId, OnItemClickListener listener) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        if(viewType==0){
            return new NewsViewHolder(inflater.inflate(R.layout.notify_main_layout,parent,false));
        }else{
...
if(values.get("NOTIFY_TYPE").equalsIgnoreCase("NEWS_COLLEGEEXPLORE")){

                if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
                    ((NewsViewHolder) holder).label.setText(Html.fromHtml(values.get("DATA3")));
                    ((NewsViewHolder) holder).newsmaintext.setText(Html.fromHtml(values.get("DATA2")));
                }
                if (Build.VERSION.SDK_INT >=Build.VERSION_CODES.N) {
                    ((NewsViewHolder) holder).label.setText(Html.fromHtml(values.get("DATA3"),Html.FROM_HTML_MODE_COMPACT));
                    ((NewsViewHolder) holder).newsmaintext.setText(Html.fromHtml(values.get("DATA2"),Html.FROM_HTML_MODE_COMPACT));
                }

                ((NewsViewHolder) holder).labelId.setText(values.get("UNIQUE_DATA1"));
                            ...
                            try {
                              ...
                              //  ((NewsViewHolder) holder).imageView.setImageBitmap(imagedata.get(position));
                               // Picasso.with(contextForFragmentadp).load("https://s3.us-east-2.amazonaws.com/elasticbeanstalk-us-east-2-083183914236/CLGS_PROJECT/AllNewsImages/news4.jpg").into( ((NewsViewHolder) holder).imageView);
                                ...
                                Picasso.with(contextForFragmentadp).load(values.get("NOTIFY_IMAGE")).resize(600, 200).into( ((NewsViewHolder) holder).imageView);
                                ViewCompat.setTransitionName(((NewsViewHolder) holder).imageView,values.get("UNIQUE_DATA1"));


                            ((NewsViewHolder) holder).imageView.setScaleType(ImageView.ScaleType.CENTER_CROP);
                        }}

                if(values.get("NOTIFY_TYPE").equalsIgnoreCase("EXAMS_COLLEGEEXPLORE") || values.get("NOTIFY_TYPE").equalsIgnoreCase("FEST_COLLEGEEXPLORE")){

                    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.N) {
                        ((NewsViewHolder) holder).label.setText(Html.fromHtml(values.get("NOTIFY_DETAIL_MSG")));
                        ((NewsViewHolder) holder).newsmaintext.setText(Html.fromHtml(values.get("NOTIFY_TITLE")));
                    }
                    if (Build.VERSION.SDK_INT >=Build.VERSION_CODES.N) {
                        ((NewsViewHolder) holder).label.setText(Html.fromHtml(values.get("NOTIFY_DETAIL_MSG"),Html.FROM_HTML_MODE_COMPACT));
                        ((NewsViewHolder) holder).newsmaintext.setText(Html.fromHtml(values.get("NOTIFY_TITLE"),Html.FROM_HTML_MODE_COMPACT));
                    }
